...
export function buildStackedBarChart(data: ChapterOccurence[], paths: string[]) {
  // Determine the series that need to be stacked.
  console.log("data", data);

  const groups = getChapterTitles(data);
  let topCharacters = getTops(data, 4);
  ...
  const height = stackedData[0].length * (barWidth*2);
  ...
  const divergingScheme = ['#045a8d','#fdd49e','#fdbb84','#fc8d59','#e34a33','#b30000'].reverse()
  // d3.schemeAccent
  const color = d3
    .scaleOrdinal<string>(divergingScheme)
    .domain(stackKeys)
    .unknown("#ccc");
  ...
  svg
    ...
    .each(function (_, i) {
      ...
      d3.select(el.parentNode)
        ...
        // .on("click", () => (document.location.href = paths[i]))
        .append(() => el);
    })
  ...
  svg
    ...
    .call(d3.axisBottom(x).tickSize(1.5).ticks(Math.round(maxEntities/100)))
    .call((g) => g.selectAll(".domain").remove());
    // .style("font", "0px times")
  ...
  //   .append("svg:a")
  //   .style("cursor", "pointer")
  //   .attr("xlink:href", "ch01.xhtml")
  //   .on("click", () => document.location.href = "ch01.xhtml")
